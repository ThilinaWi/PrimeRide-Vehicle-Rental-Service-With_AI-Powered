...
const DriverCreateModal = ({ onClose, onCreate }) => {
  ...
  const validateForm = () => {
    ...
    if (!image) {
      newErrors.image_upload = "Driver photo is required";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (image) {
        formDataToSend.append("driver_image", image);
      }
      ...
      const response = await driverService.createDriver(formDataToSend);
      ...
      if (response.success) {
        ...
        setMessage(response.message || "Error creating driver");
      }
    } catch (error) {
      ...
      const errorMessage =
        ...
        error.message ||
        "Error creating driver. Please try again.";

      setMessage(errorMessage);

      // If there are validation errors from backend, set them
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="bg-primeTeal px-6 py-4 border-b">
          <h2 className="text-2xl font-bold text-white">Create New Driver</h2>
        </div>
